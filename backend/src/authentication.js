    ...
    db.query(query, [login_username_email, login_username_email], (err, results) => {
        ...
        if (results.length === 1) {
            ...
            if (bcrypt.compareSync(login_password, user.password)) {
                ...
                return res.json({
                    message: `Xin chàochào ${user.username}! Mua sắm vui vẻ!`,
                    loggedin: true,
                    token: token,
                    user: {
                        username: user.username,
            ...
            return res.status(401).json({ message: 'Email/ Tên người dùng không hợp lệ!' });
        }
    });
};
...
const signup = (req, res, db) => {
    ...
    let errors = [];
    if (!terms) {
        errors.push('Bạn phải đồng ý với các điều khoản và điều kiện');
    }
